...
There are 6 steps to reproduced the figure above: 1) downloading the models, 2) evaluating the individual models, 3) running the uniform soup, 4) running the greedy soup, 5) running the ensemble and 6) making the plot.
...
An exception is step 1, downloading the models. If you wish to run steps 2, 3, or 4 you must first run step 1.
In addition, you must first run step 2 to cache the model predictions if you want to run step 5.
...
```bash
python main.py --eval-individual-models --cache-predictions --data-location <where data is stored> --model-location <where models are stored>
```
...
```bash
python main.py --ensemble --data-location <where data is stored> --model-location <where models are stored>
```
Note that this will first delete then rewrite the file `greedy_soup_results.jsonl`.
